...
BEGIN
    ...
    WHERE c.customer_id = ___  -- Fill in the customer_id
    GROUP BY c.first_name;
    
    -- Display the result
    RAISE NOTICE 'Customer % has % rentals', ___, ___;
END $$;
...
DECLARE
    ...
    cust_fname customer.____%TYPE;            -- Fill in the column name
    cust_lname customer.last_name%TYPE;
    cust_email customer.____%TYPE;            -- Fill in the column name
BEGIN
    -- Retrieve customer data
    SELECT customer_id, first_name, last_name, email
    INTO cust_id, cust_fname, ___, ___        -- Fill in the variables
    FROM customer
    WHERE customer_id = 5;
...
BEGIN
    ...
    SELECT COUNT(*), AVG(rental_rate), ___(rental_rate), MIN(___)
    INTO total_films, avg_rate, ___, ___  -- Fill in the variables
    FROM film;
    
    -- Display formatted report
    RAISE NOTICE '=== FILM RENTAL STATISTICS ===';
    RAISE NOTICE 'Total Films: %', total_films;
    RAISE NOTICE 'Average Rental Rate: $%', ___;
    RAISE NOTICE 'Maximum Rental Rate: $%', max_rate;
    RAISE NOTICE 'Minimum Rental Rate: $%', ___;
END $$;
...
BEGIN
    ...
    INTO ___, ___  -- Fill in the variables
    FROM film
    WHERE film_id = 1;
    
    -- Categorize based on rental rate
    IF film_rate < ___ THEN  -- Fill in the threshold
        rate_category := 'Budget';
    ELSIF film_rate >= 2.00 AND film_rate < ___ THEN  -- Fill in threshold
        rate_category := '___';  -- Fill in the category name
    ELSE
...
BEGIN
    ...
    FOR i IN 1..10 LOOP
        square_value := i * ___;  -- Calculate square (fill in)
        sum_of_squares := sum_of_squares + ___;  -- Add to sum (fill in)
    ...
    -- Display final sum
    RAISE NOTICE '';
    RAISE NOTICE 'Sum of all squares: %', ___;
END $$;
