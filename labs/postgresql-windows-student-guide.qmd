...
- **Data:** [pagila-data.sql](https://raw.githubusercontent.com/devrimgunduz/pagila/master/pagila-data.sql)

*(Right-click links → Save As... → Save to `C:\Pagila_Project`)*

Do **not** edit these official Pagila SQL files. Save them exactly as downloaded:

- `C:\Pagila_Project\pagila-schema.sql`
- `C:\Pagila_Project\pagila-data.sql`

### Create and Import the Database as `postgres`

Open `psql` as the admin user `postgres`, then run the commands in this order:
```sql
...
> **Note:** Use forward slashes (/) in `psql` paths, even on Windows. Import the schema first, then the data.

### Alternative Import from Command Prompt (CMD) or PowerShell
...
After the import succeeds, stay connected as `postgres` and create a normal user for daily TP work. In the example below, `karim` is a Moroccan first-name example.
...
Each student replaces `karim` with their own lowercase first name, for example `amina`, `youssef`, or `salma`. Choose a private password and do not share it.

To connect directly from CMD with your personal user, run:

`"C:\Program Files\PostgreSQL\<version>\bin\psql.exe" -h localhost -p 5432 -U karim -d pagila`

The same command split across two lines for readability:

```cmd
"C:\Program Files\PostgreSQL\<version>\bin\psql.exe" ^
  -h localhost -p 5432 -U karim -d pagila
```
...
- **Connection:** Open pgAdmin → Register → Server... → Host `localhost`, Port `5432`, Database `pagila`, User `karim` (or your own lowercase first name), password → **Save**
...
### C) Visual Studio Code + PostgreSQL for Visual Studio Code (Free)

**Download:** [https://marketplace.visualstudio.com/items?itemName=ms-ossdata.vscode-pgsql](https://marketplace.visualstudio.com/items?itemName=ms-ossdata.vscode-pgsql)

- **Best for:** Developers who live in VS Code
- **Extension:** **PostgreSQL for Visual Studio Code** by Microsoft (`ms-ossdata.vscode-pgsql`)
- **Connection:** Extensions → search **PostgreSQL for Visual Studio Code** (Microsoft) → Install → PostgreSQL activity view or Command Palette → add a connection → fill host/port/database/user/password
...
- **I live in VS Code** → **PostgreSQL for Visual Studio Code** extension
...
- **User:** `karim` (or your own lowercase first name); use `postgres` only for administration
...
### Your Personal Non-Superuser for Practice

Instead of always using `postgres`, create a normal user after importing Pagila. Replace `karim` with your own lowercase first name, and keep your password private:
