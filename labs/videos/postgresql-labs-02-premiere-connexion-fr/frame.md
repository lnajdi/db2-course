...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Literata for all
  display + Instrument Sans body + Geist Mono for the index/code voice on a warm-navy code surface,
...
colors:
  ...
  primary: "#0D4F68"
  primary-soft: "#E8F1F4"
  primary-border: "#A7C4CE"
  on-primary: "#FFFFFF"
  text: "#182126"
  text-muted: "#5D686E"
  text-subtle: "#788388"
  border: "#D9DEDF"
  border-strong: "#B8C1C3"
  success: "#175E4A"
  success-soft: "#E8F3EE"
  editorial: "#624A35"
  editorial-soft: "#F2ECE5"
  warning: "#8A651A"
  warning-soft: "#F8F0DC"
  danger: "#A53B35"
  danger-soft: "#F8EAE8"
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Instrument Sans", cqw: 1.5, weight: 430, lineHeight: 1.5 }
  lead:    { fontFamily: "Instrument Sans", cqw: 2.08, weight: 430, lineHeight: 1.5 }
  card-title:{ fontFamily: "Instrument Sans", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Instrument Sans", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Instrument Sans", cqw: 1.35, weight: 560, tracking: "0.18em", upper: true }
  ...
  # — display ramp (Literata 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Literata", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Literata", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Literata", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Literata", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Literata", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Literata", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Literata figure paired with a Geist Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Literata italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Literata italic, small Literata uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **Literata** (display) / **Instrument Sans** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 430, 500, 560, 600, 650, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **Literata** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Literata** carries body, leads, card titles, buttons, and
...
- **Literata** (sentence case, negative-tracked) for all display; **Literata** body/chrome; **Geist Mono** index + code.
...
- **Squint** — one Literata display moment dominates at 3–6× its neighbor.
...
- **Type** — Literata sentence-case display (negative-tracked); Literata 400 body; Geist Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Literata `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Literata `headline` 4.6cqw
...
- **Literata display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Literata body** sentence case weight 400. **Geist Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Literata figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Literata italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Literata · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Literata `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Literata 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Literata line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Literata `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Literata `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: Literata italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Literata **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Literata uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** Literata italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Literata sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **Literata, sentence case**, negative-tracked; **Literata 400** body; **Geist Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Literata figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Literata display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Literata display moment dominates.
...
- **Type** — Literata sentence-case display (negative-tracked); Literata 400 body; Geist Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
